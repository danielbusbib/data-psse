...
def process_bus_data(bus_data, generator_data, base_MVA):
    ...
    if generator_data is not None:
        generator_data = torch.tensor(generator_data, dtype=torch.float32)
...
def generate_Ybus(bus_data, branch_data):
    ...
    Ybus = ((torch.tensor(Ai.T, dtype=torch.complex128) @ torch.tensor(Yi)) +
            (torch.tensor(Aj.T, dtype=torch.complex128) @ torch.tensor(Yj)) + Ysh)
...
class Branch:
    def __init__(self, branch_data):
        ...
        self.yij = torch.cat([torch.tensor(branch_data.m_yij_d_conj_aij.values), torch.tensor(branch_data.m_yij_d_aij.values)])
        self.ysi = torch.cat([torch.tensor(branch_data.yij_p_bsi_d_aij_p2.values), torch.tensor(branch_data.yij_p_bsi.values)])
        self.gij = torch.cat([torch.real(torch.tensor(branch_data.yij.values)), torch.real(torch.tensor(branch_data.yij.values))])
        self.bij = torch.cat([torch.imag(torch.tensor(branch_data.yij.values)), torch.imag(torch.tensor(branch_data.yij.values))])
...
class System:
    ...
    def copy(self):
        return System(**{'baseMVA': self.baseMVA, 'bus': self.bus.copy(), 'branch': self.branch.copy(),
                        'nbr': self.nbr, 'nb': self.nb, 'slk_bus': self.slk_bus,
                        'Ybus': self.Ybus.clone(), 'Yii': self.Yii.clone(), 'Yij': self.Yij.clone()})
