"""
Measurement composition for IEEE 118.
"""
import numpy as np
import torch
...
class Pi:
    def __init__(self, Pi_idx, bus_data, Ybu, Yij, Yii):
        self.idx = Pi_idx if isinstance(Pi_idx, torch.Tensor) else torch.tensor(Pi_idx)
        self.bus = torch.tensor(bus_data.loc[self.idx.cpu().numpy(), 'idx_bus'].values).int()
...
class Qi:
    def __init__(self, Qi_idx, bus_data, Ybu, Yij, Yii):
        self.idx = Qi_idx if isinstance(Qi_idx, torch.Tensor) else torch.tensor(Qi_idx)
        self.bus = torch.tensor(bus_data.loc[self.idx.cpu().numpy(), 'idx_bus'].values).int()
        i, j = torch.nonzero(Ybu).T
        idx1 = np.isin(i.cpu().numpy(), self.bus.cpu().numpy())
        self.i = i[idx1]
        self.j = j[idx1]
        self.N = len(self.bus)
        Ybus = Ybu[self.bus]
...
class Cm:
    def __init__(self, Cm_idx, bus_data, bra):
        self.idx = Cm_idx if isinstance(Cm_idx, torch.Tensor) else torch.tensor(Cm_idx)
        self.i = bra.i[self.idx].int()
        self.j = bra.j[self.idx].int()
        self.N = len(self.i)
        self.A = (bra.tij[self.idx] ** 4 * (bra.gij[self.idx] ** 2 + (bra.bij[self.idx] + bra.bsi[self.idx]) ** 2)).to(dtype=torch.get_default_dtype())
        self.B = (bra.pij[self.idx] ** 2 * (bra.gij[self.idx] ** 2 + bra.bij[self.idx] ** 2)).to(dtype=torch.get_default_dtype())
        self.C = (bra.tij[self.idx] ** 2 * bra.pij[self.idx] * (bra.gij[self.idx] ** 2 + bra.bij[self.idx] * (bra.bij[self.idx] + bra.bsi[self.idx]))).to(dtype=torch.get_default_dtype())
...
class Pf:
    def __init__(self, Pf_idx, bra):
        self.idx = Pf_idx if isinstance(Pf_idx, torch.Tensor) else torch.tensor(Pf_idx)
        self.i = bra.i[self.idx].int()
        self.j = bra.j[self.idx].int()
        self.N = len(self.i)
        self.yij = bra.yij[self.idx]
...
class Qf:
    def __init__(self, Qf_idx, bra):
        self.idx = Qf_idx if isinstance(Qf_idx, torch.Tensor) else torch.tensor(Qf_idx)
        self.i = bra.i[self.idx].int()
        self.j = bra.j[self.idx].int()
        self.N = len(self.i)
        self.yij = bra.yij[self.idx]
...
class Vm:
    def __init__(self, Vm_idx, bus_data):
        self.idx = Vm_idx if isinstance(Vm_idx, torch.Tensor) else torch.tensor(Vm_idx)
        self.i = torch.tensor(bus_data.loc[self.idx.cpu().numpy(), 'idx_bus'].values).int()
        self.N = len(self.i)
